...
class ProblemController(
    ...
    @ApiResponses(value = [
        ...
        ApiResponse(responseCode = "404", description = "요청한 고유 ID에 해당하는 문제가 시스템에 존재하지 않음", content = [Content(schema = Schema(implementation = ErrorResponse::class))])
    ])
    ...
    @Operation(summary = "신규 문제 등록", description = "출제자가 작성한 새로운 문제 지문 정보와 연관된 테스트 케이스 목록을 일괄 등록합니다.")
    @ApiResponses(value = [
        ApiResponse(responseCode = "201", description = "문제가 성공적으로 생성됨"),
        ApiResponse(responseCode = "400", description = "입력 데이터 값의 유효성 검증 실패", content = [Content(schema = Schema(implementation = ErrorResponse::class))])
    ])
    @PostMapping
    fun createProblem(
    ...
    @Operation(summary = "기존 문제 및 테스트 케이스 수정", description = "특정 문제의 지문 내용, 제한 조건과 함께 동반된 테스트 케이스 목록을 원자적으로 일괄 갱신합니다.")
    @ApiResponses(value = [
        ApiResponse(responseCode = "204", description = "문제 정보 및 테스트 케이스가 성공적으로 갱신됨"),
        ApiResponse(responseCode = "404", description = "수정하려는 고유 ID의 문제가 시스템에 존재하지 않음", content = [Content(schema = Schema(implementation = ErrorResponse::class))])
    ])
    ...
    @Operation(summary = "문제 영구 삭제", description = "시스템 내에서 특정 문제를 영구적으로 파기합니다. 자식 테이블 데이터도 무결성을 위해 함께 물리 삭제됩니다.")
    @ApiResponses(value = [
        ApiResponse(responseCode = "204", description = "문제 및 하위 테스트 케이스들이 안전하게 물리 삭제됨"),
        ApiResponse(responseCode = "404", description = "삭제하려는 고유 ID의 문제가 데이터베이스에 존재하지 않음", content = [Content(schema = Schema(implementation = ErrorResponse::class))])
    ])
